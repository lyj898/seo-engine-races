 ...
 * every other stage.
 *
 * WHAT IT WILL NOT DO (default / create mode)
 ...
 *   existing reviews (<=2 sections, <=1 FAQ, or <=1 source -- the shape a
 *   hand-pasted stub review tends to have) instead of missing ones, and
 *   regenerate them through the exact same prompt/web-search/validate
 *   pipeline as a fresh review, overwriting the stub in place.
 *
 *   This exists because every content error this repo's Actions logs have
 *   caught (wrong event year, invented source types, meta_description
 *   overflow, mojibake, a stray UTF-8 BOM that crashed validate outright)
 *   came from stub reviews hand-expanded outside this pipeline and pushed
 *   straight to main. Routing revisions through here instead means they
 *   come out schema-valid and correctly cited on the first try, same as
 *   every other review this script writes -- no separate cleanup pass
 *   needed afterward.
 *
 *   Bypasses the reviewMinIntervalHours pacing gate below (that gate exists
 *   to spread out *new* articles; revising existing ones is a deliberate,
 *   explicitly-triggered maintenance action, not the hourly auto-heal), but
 *   still respects --limit/--slugs/--region the same way create mode does.
 */
...
/**
 * Is this review a stub worth regenerating? Matches the definition used to
 * audit the backlog (2026-08-09): 2 sections, 1 FAQ, 1 source is the exact
 * shape a hand-pasted thin review takes, so any one of these thresholds is
 * enough to flag it -- a review can be thin in only one dimension (e.g. 5
 * sections but a single source) and still be worth a full regeneration.
 */
function isThinReview(review) {
  return (review.sections?.length ?? 0) <= 2 || (review.faqs?.length ?? 0) <= 1 || (review.sources?.length ?? 0) <= 1;
}
...
function normalizeReview(entity, result, { revise = false } = {}) {
  ...
  return {
    ...
    sources,
    faqs,
    editorial_notes: revise
      ? `Regenerated by scripts/generate-reviews.js --revise on ${today()}, replacing a thin stub. Prose is original synthesis; quotes are short attributed excerpts. Safe to hand-edit -- future non-revise runs never overwrite an existing review.`
      : `Auto-generated by scripts/generate-reviews.js on ${today()}. Prose is original synthesis; quotes are short attributed excerpts. Safe to hand-edit -- this script never overwrites an existing review.`,
...
async function run() {
  ...
  if (explicitSlugs.length > 0) {
    ...
    // Slugs that don't exist, aren't published, or aren't eligible for the
    // current mode (already reviewed / not actually thin) are skipped with a
    // note rather than failing the run.
    const bySlug = new Map(basePool.map((e) => [e.slug, e]));
    queue = [];
    ...
    // Revise mode never applies the create-mode pacing gate below -- it's a
    // deliberately-triggered maintenance pass over the existing backlog, not
    // the hourly auto-heal that gate exists to spread out. --limit is still
    // respected, same as auto mode.
    let pool = basePool;
    if (regionArg) {
    ...
    console.log(
      `[generate-reviews] revise mode: ${existingReviews.length} reviews, ${thinEntityIds.size} thin` +
        `${regionArg ? ` (${pool.length} in region "${regionArg}")` : ''}. ` +
        `Revising up to ${limit} this run (${queue.length} candidate(s) queued for retry headroom).`
    );
  } else {
  ...
  for (const entity of queue) {
    ...
    const review = normalizeReview(entity, result, { revise });
  ...
  console.log(
    `\n[generate-reviews]${revise ? ' (revise)' : ''} done. Written: ${stats.written}, ` +
      `skipped (schema): ${stats.skippedInvalid}, skipped (uncited): ${stats.skippedUncited}, ` +
      `skipped (no substance): ${stats.noSubstance}, failed: ${stats.failed}.`
  );
